    ...
    <div className="min-h-screen flex flex-col justify-between pb-12 bg-background">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10 flex-1 w-full space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="paper-sheet p-5 space-y-1">
            ...
            <div className="text-3xl font-mono font-semibold text-foreground tracking-tight flex items-baseline gap-2">
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="paper-sheet p-8 text-center text-xs text-muted-foreground font-mono">
                  ...
                  <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2.5 flex-wrap">
                        ...
                        <span className="text-[11px] font-mono px-2 py-0.5 rounded border border-border text-muted-foreground">
                          {cls.quizMode} Fragen
                        </span>
                        <span className="text-[11px] font-mono px-2 py-0.5 rounded border border-border text-muted-foreground">
                          {cls._count.students} Codes ({rate}% fertig)
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground pt-0.5 flex-wrap">
                        {cls.teacherName ? (
                          ...
                          <button
                            ...
                            className="text-muted-foreground hover:text-foreground underline text-xs cursor-pointer font-medium"
                          >
                            + Lehrkraft eintragen
                          </button>
                        )}
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-auto">
                      <button
                        onClick={() => openEditModal(cls)}
                        className="p-2 rounded-md border border-border hover:bg-muted text-muted-foreground hover:text-foreground cursor-pointer transition-colors"
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="w-18 px-2.5 py-1.5 border border-border rounded-md bg-background text-xs text-foreground font-mono focus:outline-none focus:border-foreground"
                          />
                          <button
                      ...
                      {cls.students.length > 0 ? (
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 pt-1">
                          {cls.students.map((student) => (
                            <div
                              key={student.id}
                              className="flex items-center justify-between px-3 py-2 rounded-md border border-border bg-background text-xs font-mono"
                            >
                              <span className={student.isCompleted ? 'text-foreground font-semibold' : 'text-muted-foreground'}>
                                {student.accessKey}
                              </span>
      ...
      {editingClass && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-xs">
          <div className="w-full max-w-md bg-card p-6 border border-border space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h3 className="text-sm font-semibold text-foreground">
